...
const regionesYSusComunas = _.reduce(chile, (pais, regionData) => {
	const region = {nombre: regionData.nombre, nombreCompleto: regionData.nombreCompleto, regionID: regionData.regex, comunas: {}}
	_.forEach(regionData.provincias, provincia => _.forEach(provincia.comunas, com => {
		const {nombre, codigo} = com
		const comuna = {
			nombre,
			comunaID: codigo,
			regionID: regionData.regex
		}
		Object.defineProperty(comuna, 'region', {
			get () { return regionPorID(comuna.regionID)}
		})
		region.comunas[codigo] = comuna
	}))
	pais[regionData.regex] = region
	return pais
}, {})

const comunasYSusRegiones = _.reduce(chile, (pais, regionData) => {
	_.forEach(regionData.provincias, provincia => _.forEach(provincia.comunas, com => {
		const {nombre, codigo} = com
		const comuna = {
			nombre,
...
function todasLasComunas () {
	return comunasYSusRegiones
}

function comunasEnUnaRegion (regionID) {
	const region = _.find(chile, region => region.regex === regionID)
	const comunas = {}
	_.forEach(region.provincias, provincia => _.forEach(provincia.comunas, comuna => {
		const {nombre, nombreCompleto, codigo} = comuna
		comunas[codigo] = {nombre, nombreCompleto}
	}))
	return comunas
}


function regionIDporComunaID (comunaID) {
	// console.log('regionIDporComunaID', comunaID)
	// console.log('regionIDporComunaID comunasYSusRegiones', comunasYSusRegiones)
	const comuna = comunasYSusRegiones[comunaID]
	if (!comuna) console.error('regionIDporComunaID Comuna no encontrada: comunaID', comunaID)
	return comuna && comuna.regionID
}

function comunaPorID (comunaID) {
	if (!comunaID) throw 'comunaPorID: Falta comunaID'
	const comuna = comunasYSusRegiones[comunaID]
	if (!comuna) return null
	comuna.comunaID = comunaID
	return comuna
}

function todasLasRegiones () { return regionesYSusComunas }
function todasLasRegionesYsusComunas () { return regionesYSusComunas }
function regionPorID (regionID) { 
	const region = regionesYSusComunas[regionID]
