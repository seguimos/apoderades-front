...
const regionesYSusComunas = _.reduce(chile, (pais, regionData) => {
	...
	_.forEach(regionData.provincias, provincia => _.forEach(provincia.comunas, comuna => {
		const {nombre, codigo} = comuna
		region.comunas[codigo] = {nombre}
	}))
...
const comunasYSusRegiones = _.reduce(chile, (pais, regionData) => {
	_.forEach(regionData.provincias, provincia => _.forEach(provincia.comunas, comuna => {
		const {nombre, codigo} = comuna
		pais[codigo] = {nombre, regionID: regionData.regex, comunaID: codigo}
	}))
	return pais
}, {})
