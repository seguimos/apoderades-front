...
const cripto = {
	...
	importar: {
		firma: {
			...
			async privada (pkcs8) { return await jose.importPKCS8(pkcs8, algFirma, { extractable: false }) }
		},
		encriptacion: {
			async publica (spki) { return await jose.importSPKI(spki, algEncriptacion, { extractable: true }) },
			async privada (pkcs8) { return await jose.importPKCS8(pkcs8, algEncriptacion, { extractable: false }) }
		}
	},
	...
	async firmarToken (llavePrivada, cuerpo, { issuer }) {
		...
		try {
			// consolo.log(fx, 'cuerpo', cuerpo, 'issuer', issuer)
